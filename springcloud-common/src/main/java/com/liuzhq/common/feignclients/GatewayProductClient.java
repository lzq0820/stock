 ...
 * @Version 1.0
 */
@FeignClient("gateway-product")
public interface GatewayProductClient {

    @GetMapping("/gateway-product/test/getProduct")
    String get();

    @GetMapping("/gateway-product/param/test1")
    String test1(@RequestParam("name") String name,
                 @RequestParam("age") Integer age);

    @PostMapping("/gateway-product/param/test2")
    OpenFeignObjectParam test2(@RequestBody OpenFeignObjectParam dto);

    @PostMapping("/gateway-product/param/test3")
    OpenFeignObjectParam test3(@RequestPart("dto") OpenFeignObjectParam dto);

    @GetMapping("/gateway-product/param/test10/{id}")
    ResultModel<OpenFeignObjectParam> test10(@PathVariable("id") Integer id);

    @PostMapping("/gateway-product/param/test11")
    ResultModel<OpenFeignObjectParam> test11(@RequestBody OpenFeignObjectParam dto);

    @GetMapping("/gateway-product/timeOut/defaultTimeOut")
